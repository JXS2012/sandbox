  ...
  if (quadrotorType.compare("Pelican")==0)
    {
      ...
      kd_hover.x = 1.6*root_x;
      kd_hover.y = 1.6*root_y;
      kd_hover.z = 4;
...
{ 
  ...
  thrust_cmd = readTable(strThrustCmd);
  act_thrust = readTable(strActThrust);
  ...
  for (int i = 0; i<thrust_cmd.size(); i++)
    ROS_INFO("%.3f : %.3f",thrust_cmd[i],act_thrust[i]);
}
...
{
  int i = 4;
  while (input>act_thrust[i]-heli_mass)
    i++;
  ...
  if ((i<9)&&(i>4))
    output  = (input + heli_mass - act_thrust[i-1]) / (act_thrust[i] - act_thrust[i-1]) * 0.05 + thrust_cmd[i-1];
