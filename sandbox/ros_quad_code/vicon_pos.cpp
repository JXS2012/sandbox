...
vicon_pos::vicon_pos(std::string segmentname, std::string posname, std::string grippername){
	c = new Client;
	...
	for(i =0; i<num_subs; i++){
		...
		grippers[i] = new float[3];
		rpy[i] = new float[3];
		//quats[i] = new float[4];
	}

	segment = new string(segmentname);
	gripper = new string(grippername);
	pos_name = new string(posname);
}
...
void vicon_pos::get_gripper_coord(unsigned int index,float& x,float& y,float& z){
	x = grippers[index][0];
	y = grippers[index][1];
	z = grippers[index][2];
...
void vicon_pos::get_gripper_coord(std::string subjectname,float& x,float& y,float& z){
	get_gripper_coord(find_sub(subjectname),x,y,z);
}
...
void vicon_pos::update(){
	...
	for(i=0; i<num_subs; i++){
		...
		temp_segment = c->GetSegmentGlobalTranslation(subjects[i],*segment);
	    temp_rpy = c->GetSegmentGlobalRotationEulerXYZ(subjects[i],*segment);
	    //temp_quat = c->GetSegmentGlobalRotationQuaternion(subjects[i],*segment);

	    if(*pos_name == ""){ //if no marker name was given, use segment
	    	...
	    	temp_marker = c->GetMarkerGlobalTranslation(subjects[i],*pos_name);
	    	positions[i][0] = temp_marker.Translation[0];
	    	positions[i][1] = temp_marker.Translation[1];
	    	positions[i][2] = temp_marker.Translation[2];
	    }

	    temp_marker = c->GetMarkerGlobalTranslation(subjects[i],*gripper);
	    grippers[i][0] = temp_marker.Translation[0];
	    grippers[i][1] = temp_marker.Translation[1];
	    grippers[i][2] = temp_marker.Translation[2];
	    ...
	    /*
	    quats[i][0] = temp_quat.Rotation[0];
	    quats[i][1] = temp_quat.Rotation[1];
	    quats[i][2] = temp_quat.Rotation[2];
